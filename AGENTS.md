...
- Холодное открытие попапа стоит **209–299 мс** против **82 мс** у повторного. Причина — первая
  сборка и раскладка дерева списка плюс первая возня с окном (`orderFrontRegardless` 0.9 → 11.5 мс,
  `becameKey` 4.8 → 34.4 мс), а **не** растеризация миниатюр: их при первом открытии запрашивается
  всего ~3, и перенос растеризации вне главного потока не сдвинул холодное открытие (209–299 →
  206.9/214.4 мс). Тот же ленивый `NSImage.resized` бьёт зато по свипу (131 мс в одном окне 0.33 с),
  и это уже исправлено (`NSImage.rasterized(to:scale:)` + `ensureThumbnailImage` в detached-задаче).
  Цифры и разбор — `docs/performance-baseline.md`, Zone 1.
