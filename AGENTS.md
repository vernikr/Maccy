...
- Холодное открытие попапа было **238–334 мс** против **78–87 мс** у повторного, стало **125–136 мс**
  после `FloatingPanel.prewarm()`: он собирает, раскладывает и презентует дерево скрытой панели за
  пределами всех экранов и делает её key на один оборот runloop через ~1 с после запуска (цена —
  450–515 мс работы главного потока на старте, из них 200 мс — этот оборот). Причина холодной части —
  первая сборка/раскладка дерева и первая возня с окном (`orderFrontRegardless` ~11 → 0.7–4 мс), а
  **не** растеризация миниатюр: их при первом открытии запрашивается ~3, и перенос растеризации вне
  главного потока холодное открытие не сдвинул. Тот же ленивый `NSImage.resized` бьёт зато по свипу
  (131 мс в окне 0.33 с) — исправлено (`NSImage.rasterized(to:scale:)` + detached-задача). Цифры и
