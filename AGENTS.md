...
- Установка форка на машину — `scripts/install.sh`: собирает Release, подписывает (ad-hoc, если
  сертификата в связке нет), кладёт в `/Applications` **поверх** апстрима и **не трогает** контейнер
...
29. **Сборка без подписи молча теряет историю.** Maccy сэндбоксится (`Maccy.entitlements`), но
    ...
    (`ditto "$HOME/Library/Containers/org.p0deje.Maccy" <куда>`). Побочно: ad-hoc подпись меняется на
    каждой сборке, поэтому macOS может заново спросить Accessibility (вставка синтетической ⌘V) —
    это не регресс кода; и Sparkle такой билд обновлять не станет (валидирует обновление против
    requirements запущенного приложения), поэтому обновление локальной установки — `git pull &&
    scripts/install.sh`.
